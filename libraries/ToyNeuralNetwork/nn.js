  ...
  static load(data) {
    return tf.tidy(() => {
      ...
      for (let i = 0; i < data.weights.length; i++) {
        ...
        for (
          let j = 0;
          j < data.weights[i].weight.shape[0] * data.weights[i].weight.shape[1];
          j++
        ) {
          arr.push(data.weights[i].data[j]);
        }
        const tensor = tf.tensor2d(arr, data.weights[i].weight.shape);
      ...
      for (let i = 0; i < data.biases.length; i++) {
        ...
        for (
          let j = 0;
          j < data.biases[i].bias.shape[0] * data.biases[i].bias.shape[1];
          j++
        ) {
          arr.push(data.biases[i].data[j]);
        }
        const tensor = tf.tensor2d(arr, data.biases[i].bias.shape);
